jQuery(document).ready(function($) {
    ...
    // Products filter
    $("#products-flters li").click(function() {
        $("#products-flters li").removeClass('filter-active');
        $(this).addClass('filter-active');

        var selectedFilter = $(this).data("filter");
        $("#products-wrapper").fadeTo(100, 0);

        $(".products-item").fadeOut().css('transform', 'scale(0)');

        setTimeout(function() {
            $(selectedFilter).fadeIn(100).css('transform', 'scale(1)');
            $("#products-wrapper").fadeTo(300, 1);
        }, 300);
    });
    ...
    document.getElementById('form').addEventListener('submit', function(event) {
        ...
        if (name != '' && email != '' && message != '') {
            emailjs.sendForm(serviceID, templateID, this).then(() => {
                ...
                $('#errormessage').html("Mensaje no enviado, vuelva a intentarlo");
            });
        } else {
            $("#sendmessage").removeClass("show");
            $("#errormessage").removeClass("show");
